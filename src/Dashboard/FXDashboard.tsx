...
interface ConversionResponse {
  ...
  // ✅ Backend-calculated trend data (we just display these)
  treasuryDiff: number | null;
  treasuryPct: number | null;
  customerDiff: number | null;
...
// ==================== UTILITY FUNCTIONS (DISPLAY ONLY - NO CALCULATIONS) ====================

/**
 * ✅ SAFE: This only FORMATS numbers for display, doesn't calculate anything
 */
...
const PositionPanel: React.FC<{ 
  ...
  const fetchPositions = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/api/positions/list?v=${Date.now()}`);
  ...
  const renderPositionCard = (pos: CurrencyPosition, isActive: boolean = false) => {
    ...
    // ✅ SAFE: Just reading backend-calculated values, no math
    ...
    // ✅ SAFE: Simple comparison, no calculation
    ...
    return (
      ...
      >
        <div style={{
          ...
          gap: '16px',
          alignItems: 'center'
        }} className="position-card-content">
...
const RateWithComparison: React.FC<{
  ...
  // ✅ SAFE: Just reading backend values, no calculation
      ...
      {hasHistory && difference !== null && percentChange !== null ? (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
          ...
          <div style={{ 
            ...
            padding: '2px 6px', 
            borderRadius: '4px', 
            border: `1px solid ${isPositive ? '#10b981' : isNegative ? '#ef4444' : '#d1d5db'}` 
          }}>
...
const SearchableDropdown: React.FC<{
  ...
  return (
    <div style={{ flex: 1, position: 'relative' }} ref={dropdownRef} className="searchable-dropdown">
      <label style={{ 
        display: 'block', 
        fontSize: '17px', 
        fontWeight: '700', 
            ...
            {filteredCurrencies.map(c => (
              <div 
                ...
                style={{ 
                  padding: '10px 16px', 
                  cursor: 'pointer', 
                  backgroundColor: c.code === value ? '#eef2ff' : 'white',
      ...
      {selectedCurrency && (
        <div style={{ 
          ...
          <RateWithComparison 
            label="BNR Rate" 
            currentRate={selectedCurrency.bnrrate} 
            difference={selectedCurrency.bnrRateDifference} 
            percentChange={selectedCurrency.bnrRateChangePercent} 
            hasHistory={selectedCurrency.hasHistoricalData} 
          />
          <div style={{ height: '1px', backgroundColor: '#e5e7eb', margin: '8px 0' }} />
          <RateWithComparison 
            label="T24 Rate" 
            currentRate={isBuySide ? selectedCurrency.buyCustomerRate : selectedCurrency.sellCustomerRate} 
            difference={isBuySide ? selectedCurrency.buyRateDifference : selectedCurrency.sellRateDifference} 
            percentChange={isBuySide ? selectedCurrency.buyRateChangePercent : selectedCurrency.sellRateChangePercent} 
            hasHistory={selectedCurrency.hasHistoricalData} 
          />
        </div>
      )}
...
const FXDashboard: React.FC = () => {
  ...
  // Persist buy/sell codes to sessionStorage
  useEffect(() => {
  ...
  // Fetch currencies only once on mount
  useEffect(() => {
    if (hasInitialFetchRef.current) return;
    hasInitialFetchRef.current = true;
    ...
    const fetchCurrencies = async () => {
      ...
      try {
        ...
        setError("Unable to load exchange rates. Please refresh.");
  ...
  /**
   * ✅ BACKEND-HEAVY: This function does ZERO calculations
   * It only:
   * 1. Validates input (UI concern)
   * 2. Sends request to backend
   * 3. Displays backend response
   */
  const performCalculation = async (currentRequestId: number, isManualRate: boolean = false) => {
    const numAmount = parseFloat(amount);
    
    // ✅ SAFE: Simple UI validation, not business logic
    ...
    // ✅ REMOVED: Frontend same-currency handling - now backend does it!
    // Old code deleted:
    // if (buyCode === sellCode) { ... frontend calculations ... }

    // ✅ SAFE: Simple UI validation for manual rate
    ...
    try {
      ...
      if (currentRequestId === latestRequestRef.current) {
        // ✅ BACKEND-HEAVY: Just display what backend calculated
        setResult(data);
        
        // Note: Trading date - data.tradingDate is available if needed
      ...
      if (e.name !== 'AbortError' && currentRequestId === latestRequestRef.current) {
        setError(e.message);
        console.error('Error calculating FX:', e);
      }
    } finally {
      if (currentRequestId === latestRequestRef.current) {
        // setIsCalculating(false);
  ...
  // Initial calculation only after currencies loaded
  useEffect(() => {
    if (currencies.length === 0 || hasInitialCalcRef.current) return;
  ...
  // Recalculate only when user changes inputs
  useEffect(() => {
    if (!hasInitialCalcRef.current) return;
    
    const reqId = ++latestRequestRef.current;
    performCalculation(reqId, false);
  }, [amount, buyCode, sellCode]);

  // Debounced calculation for manual customer rate edits
  useEffect(() => {
    if (!customerRate || !isUserEditing) return;
    const reqId = ++latestRequestRef.current;
    const timer = setTimeout(() => performCalculation(reqId, true), CUSTOMER_RATE_DEBOUNCE);
  ...
  return (
    <div className="dashboard-container" style={{ 
      ...
      {/* Exchange Calculator Panel */}
      <div className="exchange-panel" style={{ 
        width: '100%', 
        maxWidth: '1000px', 
        backgroundColor: 'white', 
        borderRadius: '16px', 
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {result && (
              <div style={{
                ...
                <div style={{ fontSize: '13px', color: '#15803d', fontWeight: '650' }}>
                  💰 TRADING RATES
                </div>
        ...
        <div className="dropdowns-container" style={{ 
          display: 'flex', 
          gap: '24px', 
          marginBottom: '24px', 
            ...
            <div style={{ 
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ 
                  ...
                  {result.treasuryRate.toFixed(6)}
                </div>
                {/* ✅ BACKEND-HEAVY: Display backend-calculated treasury trend */}
                {result.treasuryPct !== null && (
                  <div style={{ 
                    fontSize: '12px', 
                    fontWeight: '700', 
                    backgroundColor: result.treasuryPct > 0 ? '#ecfdf5' : '#fef2f2', 
                    color: result.treasuryPct > 0 ? '#059669' : '#dc2626', 
                    padding: '2px 8px', 
                    borderRadius: '4px', 
                    display: 'inline-block', 
                    border: `1px solid ${result.treasuryPct > 0 ? '#10b981' : '#ef4444'}` 
                  }}>
                    {result.treasuryPct > 0 ? '+' : ''}{result.treasuryPct.toFixed(2)}%
                  </div>
                )}
              </div>
              <div style={{ textAlign: 'center' }}>
            ...
            <div className="pnl-grid" style={{ 
              ...
              <div style={{ textAlign: 'center', borderRight: '1px solid #e2e8f0' }}>
                ...
                <div style={{ 
                  fontSize: '17px', 
                  fontWeight: '700', 
                  color: getPnlStyles(result.pnlStatus).color 
                }}>
                  {/* ✅ BACKEND VALUE: No calculation! */}
                  {result.spread.toFixed(6)}
                </div>
              </div>
              <div style={{ 
                textAlign: 'center', 
                ...
                <div style={{ 
                  ...
                  fontWeight: '800', 
                  color: getPnlStyles(result.pnlStatus).color, 
                  backgroundColor: getPnlStyles(result.pnlStatus).bg, 
                  border: `1px solid ${getPnlStyles(result.pnlStatus).border}` 
                }}>
                  {/* ✅ BACKEND VALUE: No logic! */}
                  {getPnlStyles(result.pnlStatus).icon} {result.pnlStatus}
                </div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ 
                ...
                <div style={{ 
                  ...
                  {/* ✅ BACKEND VALUE: No multiplication! */}
            ...
            <div style={{ borderTop: '2px solid #e5e7eb', paddingTop: '18px' }}>
              <div style={{ 
                ...
                <span style={{ fontSize: '18px', fontWeight: '700', color: '#111827' }}>
                  {/* ✅ BACKEND VALUE */}
                  {result.conversionRate.toFixed(4)}
                </span>
              </div>
              <div style={{ 
                textAlign: 'center', 
                background: 'linear-gradient(135deg, #577db1 0%, #6366f1 100%)', 
                borderRadius: '7px', 
                padding: '12px', 
                color: 'white' 
              }}>
                <div style={{ fontSize: '35px', fontWeight: '700' }}>
                  {/* ✅ BACKEND VALUE: finalAmount calculated by BigDecimal in Java */}
                  {result.finalAmount.toFixed(3)} {result.sellCode}
                </div>
              </div>
            </div>
          </div>
      ...
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .position-card-content > div {
            text-align: center !important;
            border-bottom: 1px solid #f3f4f6;
            padding: 8px 0 !important;
          }

          .position-card-content > div:first-child {
            justify-content: center !important;
          }

          .position-card-content > div:first-child > div {
            justify-content: center !important;
          }

          .position-card-content > div:last-child {
            border-bottom: none !important;
            margin: 0 auto;
            max-width: 200px;
          }
